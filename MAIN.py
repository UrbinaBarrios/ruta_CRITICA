...
def generar_ruta():
    ...
    def open_popup():
        ...
        # Título del Reporte
        label_act = tk.Label(top, fg="#a60338", text="Reporte de Actividades", font=(
            "verdana", 12)).place(x=190, y=15)

        # LEYENDA
        terminos = tk.Label(top, fg="#a60338", text="Terminos usados en el reporte", font=(
            "verdana", 10)).place(x=790, y=70)
        termino_D = tk.Label(top, fg="#2d2da7", text="D: Duración", font=(
            "verdana", 10)).place(x=790, y=100)
        termino_ES = tk.Label(top, fg="#2d2da7", text="ES:", font=(
            "verdana", 10)).place(x=790, y=130)
        termino_ES1 = tk.Label(top, fg="#2d2da7", text="Early Start (Inicio más temprano)", font=(
            "verdana", 7)).place(x=820, y=130)
        termino_EF = tk.Label(top, fg="#2d2da7", text="EF:", font=(
            "verdana", 10)).place(x=790, y=160)
        termino_EF1 = tk.Label(top, fg="#2d2da7", text="Early Finish (Inicio más tardío)", font=(
            "verdana", 7)).place(x=820, y=160)
        termino_LS = tk.Label(top, fg="#2d2da7", text="LS:", font=(
            "verdana", 10)).place(x=790, y=190)
        termino_LS1 = tk.Label(top, fg="#2d2da7", text="Late Start (Culminación más temprana)", font=(
            "verdana", 7)).place(x=820, y=190)
        termino_LF = tk.Label(top, fg="#2d2da7", text="LF:", font=(
            "verdana", 10)).place(x=790, y=220)
        termino_LF1 = tk.Label(top, fg="#2d2da7", text="Late Finish (Culminación más tardía)", font=(
            "verdana", 7)).place(x=820, y=220)
        termino_H = tk.Label(top, fg="#2d2da7", text="H: Holgura", font=(
            "verdana", 10)).place(x=790, y=250)

        # TABLA DE REPORTE
        label_act = tk.Label(top, fg="#2d2da7", text="| Actividad", font=(
            "verdana", 10)).place(x=20, y=50)
        label_des = tk.Label(top, fg="#2d2da7", text="| Descripción", font=(
            "verdana", 10)).place(x=100, y=50)
        label_pre = tk.Label(top, fg="#2d2da7", text="| Predecesor", font=(
            "verdana", 10)).place(x=220, y=50)
        label_suc = tk.Label(top, fg="#2d2da7", text="| Sucesor", font=(
            "verdana", 10)).place(x=320, y=50)
        label_D = tk.Label(top, fg="#2d2da7", text="| D",
                           font=("verdana", 10)).place(x=420, y=50)
        label_ES = tk.Label(top, fg="#2d2da7", text="| ES",
                            font=("verdana", 10)).place(x=480, y=50)
        label_EF = tk.Label(top, fg="#2d2da7", text="| EF",
                            font=("verdana", 10)).place(x=540, y=50)
        label_LS = tk.Label(top, fg="#2d2da7", text="| LS",
                            font=("verdana", 10)).place(x=600, y=50)
        label_LF = tk.Label(top, fg="#2d2da7", text="| LF",
                            font=("verdana", 10)).place(x=660, y=50)
        label_H = tk.Label(top, fg="#2d2da7", text="| H",
                           font=("verdana", 10)).place(x=720, y=50)
        ...
        for node in G.nodes():
            ...
            tk.Label(top, fg="#292931", text="| " + str(node),
                     font=("verdana", 10)).place(x=20, y=pos_y)
            tk.Label(top, fg="#292931", text="| " +
                     str(G.nodes[node]['descripcion']), font=("verdana", 7)).place(x=100, y=pos_y)
            tk.Label(top, fg="#292931", text="| " + aux_predecesor,
                     font=("verdana", 10)).place(x=220, y=pos_y)
            tk.Label(top, fg="#292931", text="| " + aux_sucesor,
                     font=("verdana", 10)).place(x=320, y=pos_y)
            tk.Label(top, fg="#292931", text="| " +
                     str(G.nodes[node]['D']) + " días", font=("verdana", 10)).place(x=420, y=pos_y)
            tk.Label(top, fg="#292931", text="| " +
                     str(G.nodes[node]['ES']) + " días", font=("verdana", 10)).place(x=480, y=pos_y)
            tk.Label(top, fg="#292931", text="| " +
                     str(G.nodes[node]['EF']) + " días", font=("verdana", 10)).place(x=540, y=pos_y)
            tk.Label(top, fg="#292931", text="| " +
                     str(G.nodes[node]['LS']) + " días", font=("verdana", 10)).place(x=600, y=pos_y)
            tk.Label(top, fg="#292931", text="| " +
                     str(G.nodes[node]['LF']) + " días", font=("verdana", 10)).place(x=660, y=pos_y)
            tk.Label(top, fg="#292931", text="| " +
                     str(G.nodes[node]['H']) + " días", font=("verdana", 10)).place(x=720, y=pos_y)

        pos_y += 50
        btn_mostrar_grafo_CP = tk.Button(top, bg='#d15b70', text="Mostrar Grafo de la Ruta Crítica",
                                         command=mostrar_grafo, font=("verdana", 10)).place(x=780, y=500)

    open_popup()


button1 = tk.Button(text='Agregar', command=agregar_nodo,
                    bg='brown', fg='white', font=('helvetica', 9, 'bold'))
canvas1.create_window(100, 300, window=button1)

button2 = tk.Button(text='Generar ruta critica', command=generar_ruta,
